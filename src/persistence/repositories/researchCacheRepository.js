...
export function createResearchCacheRepository(db) {
  return {
    ...
    save(url, entry, ttlSeconds) {
      ...
      db.prepare(
        `INSERT INTO research_cache (url, status, title, content, error_json, fetched_at, expires_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)
         ON CONFLICT(url) DO UPDATE SET status = excluded.status, title = excluded.title, content = excluded.content,
         error_json = excluded.error_json, fetched_at = excluded.fetched_at, expires_at = excluded.expires_at`
      ).run(
        url,
        entry.status,
        entry.title ?? null,
