...
function mockEmail(record, config) {
  return {
    subject: `Quick AI SMS idea for ${record.displayName}`,
    bodyHtml: `<p>Hi there, I noticed ${record.displayName} and thought a simple AI SMS flow could help capture missed calls and guest questions.</p><p><a href="${config.business.aiSmsUrl}">AI SMS examples</a></p><p>Worth a quick look?</p>`
  };
}
...
export async function processRecord({
  ...
  try {
    ...
    if ((provider === "mock" || process.env.AI_MOCK === "true") && normalized.subject === "mock-string") {
      normalized = mockEmail(record, config);
